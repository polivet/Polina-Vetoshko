...
function soundSlider() {
    ...
    const sliderY = iconY - menuHeight;

    if (soundOpen && mouseIsPressed) {
        const newVolume = map(mouseY, sliderY + 20, sliderY + menuHeight - 20, 1, 0);
        volume = constrain(newVolume, 0, 1);
    }

    if (soundOpen) {
        fill("#ffffff");
        strokeWeight(2);
        rect(iconX, sliderY, menuWidth, menuHeight);

        strokeWeight(2);
        line(iconX + menuWidth / 2, sliderY + 20, iconX + menuWidth / 2, sliderY + menuHeight - 20);

        const dotY = map(volume, 0, 1, sliderY + menuHeight - 20, sliderY + 20);
        fill("#4a4a4a");
        circle(iconX + menuWidth / 2, dotY, 20);
    }

    image(volume > 0 ? soundon : soundoff, iconX, iconY, iconSize, iconSize);
    backMus.volume = jump.volume = death.volume = volume;
}
...
function mousePressed() {
    if (mouseX > imgX && mouseX < imgSizeX && mouseY > imgY && mouseY < imgSizeY) {
        soundOpen = true;
    } else if (soundOpen && mouseX > imgX && mouseX < imgSizeX && mouseY > imgY - 68 && mouseY < imgSizeY) {
        soundOpen = true;
    } else {
        soundOpen = false;
    }
